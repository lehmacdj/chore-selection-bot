...
--hours x = secondsToDiffTime (60 * 60 * x + 60)
hours x = secondsToDiffTime (60 * x + 1)

toUTCTime :: HourTime -> UTCTime
toUTCTime (Monday x) = realToFrac (secondsToDiffTime (17 * 60 * 60)) `addUTCTime` UTCTime startDate (hours x)
toUTCTime (Tuesday x) = realToFrac (secondsToDiffTime (18 * 60 * 60)) `addUTCTime` UTCTime startDate (hours x)
-- toUTCTime (Monday x) = nominalDay `addUTCTime` UTCTime startDate (hours x)
-- toUTCTime (Tuesday x) = (nominalDay + nominalDay) `addUTCTime` UTCTime startDate (hours x)
